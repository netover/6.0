...
| # | Rule | Reason |
|---|------|--------|
| 1 | **Tech stack decision**: Create new / complex edit → C#. Simple edit (comments, minor text) → Python. Parse/read → Python (read-only). | Wrong stack = wrong result |
| 2 | **Check dependencies first**: Run `python3 tools/oxml.py env`, if any required dependency missing, run `python3 tools/oxml.py init` | Build will fail without .NET SDK and lxml |
| 3 | **Always read first**: `CodingGuide.md` + `PitfallGuide.md` + `Quotes.cs` + `EditingGuide.md` | Contains critical patterns for both C# and Python |
| 4 | Build (C#): `python3 tools/oxml.py build` (never direct `dotnet`) | Script handles dependencies and validation |
| 5 | Validate (Python): `python3 tools/oxml.py validate <output.docx>` | Same validation as C# flow |
| 6 | Chinese quotes: `\u201c` `\u201d` (never `@""`) | Compiler treats them as string delimiters |
| 7 | Method calls: Verify parameter count against signature | Avoid CS1501 errors |
| 8 | Uncertain API: Check guides or example code | Never code from memory |
| 9 | Insert images: Read dimensions dynamically (never hardcode) | Avoid aspect ratio distortion |
...
| **Simple edits** (comments, track changes, minor text) | Python + lxml | `references/EditingGuide.md` |
| **Complex edits** (restructure, restyle, large changes) | C# + OpenXML SDK | Treat as create new |
| **Read/analyze structure** | unzip + Python lxml | Read-only, not editing |

⚠️ **Never mix these.** Never use python-docx/docx-js.

⚠️ **Common mistake**: Using Python to parse a document does NOT mean you should use Python to edit it. Parsing is read-only. For restructuring, restyling, or large changes, always use C#.
...
### Python Editing Setup (Simple Edits Only)

For comments, track changes, minor text fixes. **Complex edits → use C#.**
...
|----------|---------|--------------|
| `references/CodingGuide.md` | C# coding standards, API reference, common errors | Create new / complex edits (template apply, restructure) |
...
| `references/EditingGuide.md` | Python simple edits: comments, track changes, minor text | Simple edits only. Complex edits → C# |
