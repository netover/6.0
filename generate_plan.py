...
def _write_plan(domain_counts: Dict[str, int], output_file: Path) -> bool:
    ...
    try:
        ...
        print(f"ERROR: Falha ao guardar o plano {output_file.name}: {e}", file=sys.stderr)
        return False
...
def generate_plan() -> None:
    ...
    if _write_plan(domain_counts, output_file):
        print(f"✅ Plano gerado com sucesso em '{output_file.absolute()}' ({len(domain_counts)} steps).")
