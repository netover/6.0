...
@admin_router.get(
    "/settings",
    response_class=HTMLResponse,
    summary="Settings Manager",
    dependencies=[Depends(verify_admin_credentials)],
)
async def settings_manager_page(request: Request) -> HTMLResponse:
    """Serve the comprehensive settings management page.

    Renders the HTML interface for managing ALL system settings
    ...
    try:
        ...
        if isinstance(e, (SystemExit, KeyboardInterrupt, asyncio.CancelledError, TypeError, KeyError, AttributeError, IndexError)):
            raise
        logger.error("Failed to render settings manager: %s", e, exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to render settings manager",
        ) from e
