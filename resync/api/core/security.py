...
async def get_current_user(token: str=Depends(oauth2_scheme)):
    ...
    return {'user_id': payload.get('sub'), 'username': payload.get('username', payload.get('sub')), 'role': payload.get('role', 'user'), 'permissions': payload.get('permissions', [])}