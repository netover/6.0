...
class CORSPolicy(BaseModel):
    ...
    allowed_origins: list[str] = Field(default=[], description='List of allowed origins. Use specific domains in production, wildcards only in development.')
    allowed_methods: list[str] = Field(default=['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS'], description='List of allowed HTTP methods.')
    allowed_headers: list[str] = Field(default=['Content-Type', 'Authorization', 'X-Requested-With'], description='List of allowed headers.')
    allow_credentials: bool = Field(default=False, description='Whether to allow credentials in CORS requests.')
    max_age: int = Field(default=86400, description='Maximum age in seconds for preflight cache.')
    allow_all_origins: bool = Field(default=False, description='Whether to allow all origins (development only).')
    log_violations: bool = Field(default=True, description='Whether to log CORS violations for security monitoring.')
    origin_regex_patterns: list[str] = Field(default=[], description='Regex patterns for dynamic origin validation.')
    ...
    def validate_origin(cls, v, info):
        ...
        for origin in v:
            if environment == Environment.PRODUCTION and '*' in origin:
                raise ValueError('Wildcard origins are not allowed in production. Use specific domain names instead.')
            if origin != '*' and (not cls._is_valid_origin_format(origin)):
                raise ValueError(f'Invalid origin format: {origin}. Expected format: http(s)://domain.com or http(s)://domain.com:port')
    ...
    def validate_method(cls, v):
        ...
        for method in v:
            if method not in allowed_methods:
                raise ValueError(f"Invalid HTTP method: {method}. Allowed methods: {', '.join(allowed_methods)}")
    ...
    def get_cors_config_dict(self) -> dict:
        ...
        """
        return {'allow_origins': self.allowed_origins if not self.allow_all_origins else ['*'], 'allow_methods': self.allowed_methods, 'allow_headers': self.allowed_headers, 'allow_credentials': self.allow_credentials, 'max_age': self.max_age}
...
class CORSConfig(BaseModel):
    ...
    """
    development: CORSPolicy = Field(default_factory=lambda: CORSPolicy(environment=Environment.DEVELOPMENT, allowed_origins=['*'], allow_all_origins=True, allow_credentials=False, log_violations=True), description='CORS policy for development environment')
    production: CORSPolicy = Field(default_factory=lambda: CORSPolicy(environment=Environment.PRODUCTION, allowed_origins=[], allow_all_origins=False, allow_credentials=False, log_violations=True, origin_regex_patterns=[]), description='CORS policy for production environment')
    test: CORSPolicy = Field(default_factory=lambda: CORSPolicy(environment=Environment.TEST, allowed_origins=['http://localhost:3000', 'http://localhost:8000'], allow_all_origins=False, allow_credentials=True, log_violations=True), description='CORS policy for test environment')
