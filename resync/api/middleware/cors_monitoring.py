...
class CORSMonitor:
    ...
    def __init__(self):
        ...
        self.requests = deque(maxlen=2000)
    ...
    def monitor_request(self, request: Request, operation: CORSOperation=CORSOperation.REQUEST) -> dict[str, Any]:
        ...
        cors_info = {'timestamp': datetime.now(timezone.utc).isoformat(), 'origin': origin, 'method': method, 'requested_headers': requested_headers, 'access_control_method': access_control_method, 'path': request.url.path, 'user_agent': request.headers.get('user-agent'), 'operation': operation.value}
        self.requests.append(cors_info)
        return cors_info

    def log_violation(self, origin: str, path: str, details: str='', method: str='', requested_headers: str='') -> None:
        """
        ...
        """
        violation = {'timestamp': datetime.now(timezone.utc).isoformat(), 'origin': origin, 'path': path, 'method': method, 'requested_headers': requested_headers, 'details': details}
        self.violations.append(violation)
        self.monitor_request(type('MockRequest', (), {'headers': {'origin': origin, 'user-agent': 'CORS-Monitor'}, 'url': type('URL', (), {'path': path})(), 'method': method})(), CORSOperation.VIOLATION)
        log_with_correlation(logging.WARNING, f'CORS violation detected: {origin} accessing {path}', origin=origin, path=path, details=details, method=method)
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {'total_requests': total_requests, 'total_violations': total_violations, 'unique_origins': unique_origins, 'blocked_origins_count': blocked_origins_count, 'violation_rate': total_violations / total_requests if total_requests > 0 else 0, 'last_violation': self.violations[-1] if self.violations else None}
        ...
        async def send_wrapper(message):
            if message['type'] == 'http.response.start' and request.method == 'OPTIONS':
                ...
                if origin:
                    ...
                    is_allowed = any((allowed_origin == '*' or origin == allowed_origin or (allowed_origin.startswith('.') and (origin == allowed_origin[1:] or origin.endswith('.' + allowed_origin[1:]))) for allowed_origin in self.allowed_origins))
                    if not is_allowed:
        ...
        if origin:
            try:
                ...
                if not parsed.scheme or not parsed.netloc:
                    self.cors_monitor.log_violation(origin, request.url.path, 'Invalid origin format', request.method, request.headers.get('access-control-request-headers', ''))
            except Exception as e:
                logger.error('exception_caught', exc_info=True, extra={'error': str(e)})
                self.cors_monitor.log_violation(origin, request.url.path, f'Malformed origin URL: {str(e)}', request.method, request.headers.get('access-control-request-headers', ''))
