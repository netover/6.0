...
from resync.core.utils.error_utils import ErrorResponseBuilder, create_error_response_from_exception, create_json_response_from_error, extract_validation_errors, generate_correlation_id, log_error_response
...
class GlobalExceptionHandlerMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable[[Request], Any]) -> Response:
        ...
        try:
            ...
            if processing_time > 1.0:
                logger.warning(f'Slow request detected: {request.method} {request.url.path} took {processing_time:.2f}s', extra={'correlation_id': correlation_id})
            return response
    ...
    async def _handle_validation_error(self, request: Request, exc: RequestValidationError, correlation_id: str) -> JSONResponse:
    ...
    async def _handle_resync_exception(self, request: Request, exc: ResyncException, correlation_id: str) -> JSONResponse:
    ...
    async def _handle_generic_exception(self, request: Request, exc: Exception, correlation_id: str) -> JSONResponse:
