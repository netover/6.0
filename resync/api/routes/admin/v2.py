...
router = APIRouter(prefix='/admin', tags=['Admin 2.0'], dependencies=[Depends(verify_admin_credentials)])
...
async def get_realtime_health():
    ...
    try:
        ...
        services.append(ServiceHealth(name='redis', status=HealthStatus.HEALTHY if redis_health.get('healthy') else HealthStatus.UNHEALTHY, latency_ms=redis_health.get('latency_ms'), message=redis_health.get('message'), last_check=datetime.now(timezone.utc), details={'connected': redis_health.get('connected', False), 'memory_used': redis_health.get('memory_used'), 'clients': redis_health.get('connected_clients')}))
        if not redis_health.get('healthy'):
            overall_healthy = False
    except Exception as e:
        services.append(ServiceHealth(name='redis', status=HealthStatus.UNKNOWN, message=str(e), last_check=datetime.now(timezone.utc)))
    ...
    try:
        ...
        services.append(ServiceHealth(name='database', status=HealthStatus.UNKNOWN, message=str(e), last_check=datetime.now(timezone.utc)))
    ...
    try:
        ...
        services.append(ServiceHealth(name='tws', status=HealthStatus.HEALTHY if is_healthy else HealthStatus.DEGRADED, latency_ms=tws_metrics.get('avg_latency_ms'), message='Circuit breaker OPEN' if not is_healthy else None, last_check=datetime.now(timezone.utc), details={'success_rate': tws_metrics.get('success_rate'), 'total_requests': tws_metrics.get('total_requests'), 'circuit_breaker_state': tws_metrics.get('circuit_breaker_state')}))
        if not is_healthy:
            overall_healthy = False
    except Exception as e:
        services.append(ServiceHealth(name='tws', status=HealthStatus.UNKNOWN, message=str(e), last_check=datetime.now(timezone.utc)))
    ...
    try:
        ...
        services.append(ServiceHealth(name='llm', status=HealthStatus.UNHEALTHY if all_open else HealthStatus.HEALTHY, message='All providers circuit breakers open' if all_open else None, last_check=datetime.now(timezone.utc), details={'total_requests': llm_metrics.get('total_requests'), 'fallback_rate': llm_metrics.get('fallback_rate'), 'circuit_breakers': cb_states}))
        if all_open:
            overall_healthy = False
    except Exception as e:
        services.append(ServiceHealth(name='llm', status=HealthStatus.UNKNOWN, message=str(e), last_check=datetime.now(timezone.utc)))
    ...
    try:
        ...
        uptime = 0
    return SystemHealthResponse(overall_status=HealthStatus.HEALTHY if overall_healthy else HealthStatus.DEGRADED, services=services, timestamp=datetime.now(timezone.utc), uptime_seconds=uptime, version='5.4.2')

@router.get('/resilience/status')
async def get_resilience_status():
    """Get status of all resilience components"""
    response = {'circuit_breakers': [], 'redis_strategy': None, 'degraded_endpoints': [], 'timestamp': datetime.now(timezone.utc).isoformat()}
    try:
        ...
        for name in CircuitBreakers:
            try:
                ...
                if is_open:
                    ...
                    if is_critical:
                        critical_open += 1
                breakers.append(CircuitBreakerStatus(name=name.value, state=state, failure_count=metrics.get('failure_count', 0), success_count=metrics.get('success_count', 0), last_failure=metrics.get('last_failure'), last_success=metrics.get('last_success'), threshold=config.get('threshold', 5), recovery_timeout=config.get('recovery_timeout', 60), is_critical=is_critical))
            except Exception as e:
                logger.warning('Failed to get breaker %s', name, extra={'error': str(e)})
        return CircuitBreakerListResponse(breakers=breakers, total=len(breakers), open_count=open_count, critical_open_count=critical_open)
    except ImportError:
        return CircuitBreakerListResponse(breakers=[], total=0, open_count=0, critical_open_count=0)
    ...
    try:
        ...
        for cb in CircuitBreakers:
            if cb.value == name or cb.name == name:
                ...
                break
        if not breaker_enum:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Circuit breaker '{name}' not found")
        registry = get_registry()
        ...
        return {'success': True, 'message': f"Circuit breaker '{name}' has been reset", 'new_state': 'CLOSED'}
    except HTTPException:
        raise
    except Exception as e:
        if isinstance(e, (TypeError, KeyError, AttributeError, IndexError)):
            raise
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail='Failed to reset circuit breaker. Check server logs for details.') from e
...
async def update_resilience_config(config: ResilienceConfigRequest):
    ...
    try:
        ...
        if config.fail_fast_enabled is not None:
            event = await manager.set('redis.fail_fast_enabled', config.fail_fast_enabled, user='admin')
            changes.append(event.key)
        if config.fail_fast_timeout is not None:
            event = await manager.set('redis.fail_fast_timeout', config.fail_fast_timeout, user='admin')
            changes.append(event.key)
        return {'success': True, 'updated': changes, 'restart_required': manager.requires_restart()}
    except Exception as e:
        if isinstance(e, (TypeError, KeyError, AttributeError, IndexError)):
            raise
...
async def get_chunking_config():
    ...
    try:
        from resync.services.config_manager import get_config_manager
        manager = await get_config_manager()
        return ChunkingConfig(strategy=manager.get('rag.chunking_strategy', 'tws_optimized'), chunk_size=manager.get('rag.chunk_size', 512), chunk_overlap=manager.get('rag.chunk_overlap', 50), preserve_structure=manager.get('rag.preserve_structure', True), extract_metadata=manager.get('rag.extract_metadata', True))
    except Exception as e:
        if isinstance(e, (TypeError, KeyError, AttributeError, IndexError)):
            raise
        return ChunkingConfig(strategy='tws_optimized', chunk_size=512, chunk_overlap=50)
...
async def update_chunking_config(config: ChunkingConfig):
    ...
    try:
        ...
        if config.strategy not in valid_strategies:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f'Invalid strategy. Must be one of: {valid_strategies}')
        manager = await get_config_manager()
        ...
        return {'success': True, 'config': config.model_dump(), 'message': 'Configuration updated. Run reindex to apply to existing documents.', 'restart_required': manager.requires_restart()}
    except HTTPException:
        raise
    except Exception as e:
        if isinstance(e, (TypeError, KeyError, AttributeError, IndexError)):
...
async def start_reindex(request: ReindexRequest, background_tasks: BackgroundTasks):
    ...
    job_status = ReindexStatus(job_id=job_id, status='pending', progress=0.0, documents_processed=0, documents_total=0, started_at=datetime.now(timezone.utc))
    _reindex_jobs[job_id] = job_status
    background_tasks.add_task(_run_reindex, job_id, request.strategy, request.chunk_size, request.documents)
    return {'job_id': job_id, 'status': 'pending', 'message': 'Reindex job started. Track progress via /admin/rag/reindex/{job_id}'}

async def _run_reindex(job_id: str, strategy: str | None, chunk_size: int | None, documents: list[str] | None):
...
async def toggle_maintenance_mode(request: MaintenanceModeRequest):
    ...
    try:
        ...
        return {'success': True, 'maintenance_mode': request.enabled, 'message': request.message if request.enabled else 'Maintenance mode disabled'}
    except Exception as e:
        if isinstance(e, (TypeError, KeyError, AttributeError, IndexError)):
            raise
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail='Failed to toggle maintenance mode. Check server logs for details.') from e
...
async def restore_from_backup(request: RestoreRequest):
    ...
    try:
        ...
        if not manager.get('system.maintenance_mode', False):
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='System must be in maintenance mode before restore')
        if not request.confirm:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Restore operation requires explicit confirmation')
        backup_dir = Path(__file__).parent.parent.parent.parent.parent / 'backup'
        backup_file = backup_dir / f'{request.backup_id}'
        if not backup_file.exists() and (not (backup_dir / f'audit_backup_{request.backup_id}.json').exists()):
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Backup '{request.backup_id}' not found")
        return {'status': 'manual_required', 'message': 'PostgreSQL restore should be performed manually', 'instructions': ['1. Stop the application: systemctl stop resync', f'2. Restore database: pg_restore -d resync {request.backup_id}', f'3. Restore config: cp backup/{request.backup_id}/config/* config/', '4. Start the application: systemctl start resync'], 'backup_id': request.backup_id}
    except HTTPException:
        raise
    except Exception as e:
        if isinstance(e, (TypeError, KeyError, AttributeError, IndexError)):
            raise
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail='Restore operation failed. Check server logs for details.') from e
...
async def check_restart_required():
    ...
    try:
        from resync.services.config_manager import get_config_manager
        manager = await get_config_manager()
        return {'restart_required': manager.requires_restart(), 'pending_changes': list(manager.get_pending_restarts()), 'urgency': manager.get_restart_requirement().value}
    except Exception as e:
        if isinstance(e, (TypeError, KeyError, AttributeError, IndexError)):
            raise
...
async def get_all_config():
    ...
    try:
        ...
        return {'configs': {key: {'value': cv.value, 'source': cv.source.value, 'editable': cv.editable, 'restart_required': cv.restart_required.value, 'description': cv.description, 'last_modified': cv.last_modified.isoformat() if cv.last_modified else None, 'modified_by': cv.modified_by} for key, cv in configs.items()}, 'restart_required': manager.requires_restart(), 'pending_restarts': list(manager.get_pending_restarts())}
    except Exception as e:
        if isinstance(e, (TypeError, KeyError, AttributeError, IndexError)):
            raise
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail='Failed to get config. Check server logs for details.') from e
...
async def update_config(key: str, value: Any):
    ...
    try:
        ...
        return {'success': True, 'key': key, 'value': value, 'restart_required': event.restart_required.value}
    except ValueError as e:
        logger.error('request_failed', exc_info=True, extra={'error': str(e), 'error_type': type(e).__name__})
