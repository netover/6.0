...
class CacheMetrics(BaseModel):
    ...
    oldest_entry_age_hours: float | None = Field(default=None, description='Idade da entrada mais antiga em horas')
...
class RouterMetrics(BaseModel):
    ...
    avg_classification_ms: float | None = Field(description='Tempo médio de classificação em ms')
...
class ValidatorMetrics(BaseModel):
    ...
    avg_validation_ms: float | None = Field(description='Tempo médio de validação em ms')
...
class MetricsStore:
    ...
    def record_validation(self, validation_type: str, success: bool):
        self._validations += 1
        self._validation_type_counts[validation_type] = self._validation_type_counts.get(validation_type, 0) + 1
        if not success:
...
async def get_dashboard_metrics(store: MetricsStore=Depends(get_metrics_store_dep)) -> DashboardMetrics:
    """
    ...
    cache_metrics = CacheMetrics(hit_rate=router_hit_ratio, total_entries=router_total, memory_mb=memory_mb * 0.3, avg_latency_ms=snapshot.get('router_cache', {}).get('avg_latency_ms'), hits_last_hour=router_hits, misses_last_hour=router_misses)
    router_stats = snapshot.get('router', {})
    router_metrics = RouterMetrics(accuracy=router_stats.get('accuracy'), fallback_rate=router_stats.get('fallback_rate', 0.0), avg_classification_ms=router_stats.get('avg_duration_ms'), total_classifications=router_stats.get('total', 0), low_confidence_count=router_stats.get('low_confidence_count', 0), top_intents=router_stats.get('top_intents', {}))
    from resync.knowledge.config import CFG
    reranker_metrics = RerankerMetrics(enabled=CFG.enable_cross_encoder, model=CFG.cross_encoder_model, avg_rerank_ms=None, docs_processed=None, docs_filtered=0, filter_rate=0.3, threshold=CFG.cross_encoder_threshold)
    validator_metrics = ValidatorMetrics(total_validations=store._validations, successful_validations=store._validations - store._validation_failures, failed_validations=store._validation_failures, avg_validation_ms=None, validation_types=store._validation_type_counts)
    warming_metrics = WarmingMetrics(last_warm=None, queries_warmed=0, queries_skipped=0, errors=0, duration_seconds=0.0, is_warming=False)
    try:
        from resync.core.cache.cache_warmer import get_cache_warmer
        warmer = get_cache_warmer()
        stats = warmer.get_stats()
        warming_metrics = WarmingMetrics(last_warm=datetime.fromisoformat(stats['last_warm']) if stats.get('last_warm') else None, queries_warmed=stats.get('queries_cached', 0), queries_skipped=stats.get('queries_skipped', 0), errors=stats.get('errors', 0), duration_seconds=stats.get('duration_seconds', 0.0), is_warming=warmer.is_warming)
    except Exception as e:
        import logging
        logging.getLogger(__name__).warning(f'Failed to get warming stats: {e}. Using defaults.')
    ...
    return DashboardMetrics(timestamp=datetime.now(timezone.utc), cache=cache_metrics, router=router_metrics, reranker=reranker_metrics, validators=validator_metrics, warming=warming_metrics, system=system_metrics)
...
async def get_cache_metrics() -> CacheMetrics:
    ...
    router_total = router_hits + router_misses
    router_hit_ratio = router_hits / router_total if router_total > 0 else 0.0
    return CacheMetrics(hit_rate=router_hit_ratio, total_entries=router_total, memory_mb=snapshot.get('system', {}).get('memory_mb', 0.0) * 0.3, avg_latency_ms=router_cache.get('avg_latency_ms'), hits_last_hour=router_hits, misses_last_hour=router_misses)

@router.get('/cache/history', response_model=HistoricalData)
async def get_cache_history(hours: int=Query(default=24, ge=1, le=168, description='Período em horas'), interval: str=Query(default='1h', pattern='^(5m|15m|1h|6h|1d)$', description='Intervalo de agregação')) -> HistoricalData:
    """
    Retorna histórico de métricas do cache.

    Útil para visualização de tendências ao longo do tempo.
    """
    return HistoricalData(metric='cache_hit_rate', period_hours=hours, interval=interval, data_points=[])

@router.get('/router/intent-distribution')
async def get_intent_distribution(hours: int=Query(default=24, ge=1, le=168)) -> dict[str, Any]:
    ...
    return {'period_hours': hours, 'distribution': distribution, 'total_classifications': total, 'top_intents': intent_counts}

@router.post('/cache/warm')
async def trigger_cache_warming(priority: int=Query(default=1, ge=1, le=3, description='Nível de prioridade'), include_history: bool=Query(default=False, description='Incluir queries do histórico')) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if include_history:
            ...
            result = {'static_queries': static_count, 'critical_jobs': critical_count, 'total': static_count + critical_count, 'stats': warmer.get_stats()}
        return {'status': 'completed', 'message': 'Cache warming executado com sucesso', 'result': result}
    except HTTPException:
        raise
    except Exception as e:
        if isinstance(e, (TypeError, KeyError, AttributeError, IndexError)):
            raise
        raise HTTPException(status_code=500, detail='Erro no cache warming. Check server logs for details.') from e
...
async def get_warming_queries() -> dict[str, Any]:
    ...
    for q in warmer.STATIC_QUERIES:
        queries_by_priority[q.priority].append({'query': q.query, 'category': q.category, 'expected_intent': q.expected_intent})
        if q.category not in queries_by_category:
            queries_by_category[q.category] = []
        queries_by_category[q.category].append(q.query)
    return {'total_queries': len(warmer.STATIC_QUERIES), 'by_priority': {f'priority_{p}': {'count': len(queries), 'queries': queries} for p, queries in queries_by_priority.items()}, 'by_category': {cat: {'count': len(queries), 'queries': queries} for cat, queries in queries_by_category.items()}, 'counts': warmer.get_static_queries_count()}
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            router_total = router_hits + router_misses
            router_hit_ratio = router_hits / router_total if router_total > 0 else 0.0
            cache_metrics = CacheMetrics(hit_rate=router_hit_ratio, total_entries=router_total, memory_mb=memory_mb * 0.3, avg_latency_ms=router_cache.get('avg_latency_ms'), hits_last_hour=router_hits, misses_last_hour=router_misses)
            router_stats = snapshot.get('router', {})
            router_metrics = RouterMetrics(accuracy=router_stats.get('accuracy'), fallback_rate=router_stats.get('fallback_rate', 0.0), avg_classification_ms=router_stats.get('avg_duration_ms'), total_classifications=router_stats.get('total', 0), low_confidence_count=router_stats.get('low_confidence_count', 0), top_intents=router_stats.get('top_intents', {}))
            from resync.knowledge.config import CFG
            reranker_metrics = RerankerMetrics(enabled=CFG.enable_cross_encoder, model=CFG.cross_encoder_model, avg_rerank_ms=None, docs_processed=None, docs_filtered=0, filter_rate=0.3, threshold=CFG.cross_encoder_threshold)
