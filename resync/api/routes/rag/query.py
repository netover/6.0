...
def validate_file(file: UploadFile) -> None:
    ...
    try:
        validation_model = FileUploadValidation(filename=file.filename or '', content_type=file.content_type or '', size=file.size or 0)
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail='File validation failed. Check server logs for details.') from e
...
async def save_upload_file(upload_file: UploadFile, destination: Path) -> str:
    ...
    try:
        ...
        if isinstance(e, (TypeError, KeyError, AttributeError, IndexError)):
            raise
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail='Failed to save file. Check server logs for details.') from e

async def process_rag_document(rag_service: RAGIntegrationService, file_id: str, filename: str, content: str, tags: list[str]):
    """Background task to process document for RAG."""
    try:
        await rag_service.ingest_document(file_id=file_id, filename=filename, content=content, tags=tags)
    except Exception as e:
        if isinstance(e, (TypeError, KeyError, AttributeError, IndexError)):
            raise
        import logging
...
async def upload_rag_file(background_tasks: BackgroundTasks, file: UploadFile=File(...), tags: str=Query(default='', description='Comma-separated tags'), current_user: dict=Depends(get_current_user), logger_instance=Depends(get_logger), rag_service: RAGIntegrationService=Depends(get_rag)):
    """
    ...
    try:
        ...
        background_tasks.add_task(process_rag_document, rag_service, file_id, original_filename, content, tag_list)
        upload_response = FileUploadResponse(filename=original_filename, status='processing', file_id=file_id, upload_time=datetime.now(timezone.utc).isoformat())
        logger_instance.info('rag_file_uploaded', user_id=current_user.get('user_id'), filename=original_filename, file_id=file_id, file_size=file.size, tags=tag_list)
        ...
        if isinstance(e, (TypeError, KeyError, AttributeError, IndexError)):
            raise
        logger_instance.error('rag_upload_error', error=str(e), filename=Path(file.filename or '').name, user_id=current_user.get('user_id'))
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail='Failed to process file upload') from e

@router.get('/rag/search')
async def search_rag(query: str=Query(..., description='Search query', min_length=1), top_k: int=Query(default=10, ge=1, le=100, description='Number of results'), current_user: dict=Depends(get_current_user), logger_instance=Depends(get_logger), rag_service: RAGIntegrationService=Depends(get_rag)):
    """
    ...
    try:
        ...
        logger_instance.info('rag_search', user_id=current_user.get('user_id'), query=query[:50], results_count=len(results))
        return {'query': query, 'results': [{'chunk_id': r.chunk_id, 'doc_id': r.doc_id, 'content': r.content, 'score': r.score, 'metadata': r.metadata} for r in results], 'total': len(results)}
    except Exception as e:
        if isinstance(e, (TypeError, KeyError, AttributeError, IndexError)):
            raise
...
async def list_rag_files(status_filter: str | None=Query(default=None, description='Filter by status'), limit: int=Query(default=100, ge=1, le=1000), current_user: dict=Depends(get_current_user), logger_instance=Depends(get_logger), rag_service: RAGIntegrationService=Depends(get_rag)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail='Failed to list RAG files') from e
...
async def delete_rag_file(file_id: str, current_user: dict=Depends(get_current_user), logger_instance=Depends(get_logger), rag_service: RAGIntegrationService=Depends(get_rag)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail='Failed to delete RAG file') from e
