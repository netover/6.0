...
def create_error_response(error_type: str, message: str, status_code: int, details: dict[str, Any] | None=None) -> dict[str, Any]:
    """Create standardized error response"""
    response = {'error': {'type': error_type, 'message': message, 'status_code': status_code, 'timestamp': get_current_timestamp()}}
    if details:
        response['error']['details'] = details
    return response

def create_success_response(data: Any, message: str | None=None, metadata: dict[str, Any] | None=None) -> dict[str, Any]:
...
def parse_user_agent(user_agent: str) -> dict[str, str]:
    ...
    if 'mobile' in ua_lower or 'android' in ua_lower:
        ...
        device = 'bot'
    return {'raw': user_agent[:200], 'browser': browser, 'os': os_name, 'device': device}