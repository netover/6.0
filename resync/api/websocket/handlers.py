...
class ConnectionManager:
    ...
    def disconnect(self, websocket: WebSocket):
        """Disconnect WebSocket - note: cannot be async but uses try-lock pattern"""
        # Try to acquire lock, but don't await since this might be called from sync context
        try:
            loop = asyncio.get_running_loop()
            # If we're in async context, we'd need to handle this differently
            # For sync disconnect, we do a best-effort lock
            import threading
            if threading.current_thread() == threading.main_thread():
                # In sync context (e.g., exception handler), just do the operation
                pass
        except RuntimeError:
...
async def websocket_handler(websocket: WebSocket, agent_id: str, token: str | None = None):
