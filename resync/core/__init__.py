...
from resync.core.exceptions import AgentExecutionError, AuditError, AuthenticationError, BaseAppException, DatabaseError, InvalidConfigError, LLMError, PoolExhaustedError, RedisConnectionError, ToolProcessingError
...
class CoreBootManager:
    ...
    def __init__(self):
        ...
        self._global_correlation_context = {'boot_id': self._correlation_id, 'environment': 'unknown', 'security_level': 'unknown', 'start_time': time.time(), 'events': collections.deque(maxlen=100)}
    ...
    def get_boot_status(self) -> dict[str, Any]:
        ...
        with self._boot_lock:
            return {'components': list(self._components.keys()), 'boot_times': self._boot_times.copy(), 'health_status': self._health_status.copy(), 'correlation_id': self._correlation_id}
    ...
    def add_global_event(self, event: str, data: dict[str, Any] | None=None) -> None:
        ...
        with self._boot_lock:
            ...
            self._global_correlation_context['events'].append({'timestamp': time.time(), 'event': sanitized_event, 'data': sanitized_data})
    ...
    def get_environment_tags(self) -> dict[str, Any]:
        ...
        return {'is_mock': getattr(self, '_is_mock', False), 'mock_reason': getattr(self, '_mock_reason', None), 'boot_id': self._correlation_id, 'component_count': len(self._components)}
...
class EnvironmentDetector:
    ...
    def detect_environment(self) -> dict[str, Any]:
        ...
        return {'platform': os.name, 'is_ci': bool(os.environ.get('CI')), 'has_internet': self._check_internet_access(), 'temp_dir': os.environ.get('TEMP', os.environ.get('TMP', tempfile.gettempdir()))}
    ...
    def validate_environment(self) -> bool:
        ...
        try:
            ...
            env_ok = True
            self._validation_cache = {'result': env_ok, 'timestamp': current_time, 'details': {}}
...
def _validate_environment():
    ...
    try:
        ...
        if not env_detector.validate_environment():
            log.warning('Environment validation failed - system may not be secure', extra={'correlation_id': bm.get_global_correlation_id()})
    except (ImportError, AttributeError, OSError, RuntimeError) as e:
        try:
            log = _get_logger()
            log.warning('Environment validation failed: %s', e)
        except (ImportError, RuntimeError):
            logger.debug('suppressed_exception', exc_info=True, extra={'error': str(e)})
__all__ = ['CoreBootManager', 'EnvironmentDetector', 'get_boot_manager', 'get_global_correlation_id', 'get_environment_tags', 'add_global_trace_event', 'AuditError', 'DatabaseError', 'PoolExhaustedError', 'ToolProcessingError', 'BaseAppException', 'InvalidConfigError', 'AgentExecutionError', 'AuthenticationError', 'LLMError', 'RedisConnectionError']
