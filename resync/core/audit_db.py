...
__all__ = ['AuditDB', 'get_audit_db', 'log_audit_action', 'add_audit_records_batch', 'add_audit_records_batch_async']
...
class AuditDB:
    ...
    def to_record_dict(entry: AuditEntry) -> dict:
        ...
        return {'id': entry.id, 'action': entry.action, 'user_id': entry.user_id, 'entity_type': entry.entity_type, 'entity_id': entry.entity_id, 'old_value': entry.old_value, 'new_value': entry.new_value, 'ip_address': entry.ip_address, 'user_agent': entry.user_agent, 'timestamp': entry.timestamp.isoformat() if entry.timestamp else None, 'metadata': entry.metadata_}

    def get_records(self, limit: int=100, offset: int=0) -> list[dict]:
        """Backward-compatible sync accessor for legacy admin endpoints."""
        logger.warning('auditdb_get_records_sync_shim_used', extra={'limit': limit, 'offset': offset})
        try:
            ...
            raise RuntimeError('get_records() cannot be used inside an active event loop; use get_recent_actions()')

    async def log_action(self, action: str, user_id: str | None=None, entity_type: str | None=None, entity_id: str | None=None, old_value: dict | None=None, new_value: dict | None=None, ip_address: str | None=None, user_agent: str | None=None, metadata: dict | None=None) -> AuditEntry:
        """Log an audit action."""
        return await self._repo.log_action(action=action, user_id=user_id, entity_type=entity_type, entity_id=entity_id, old_value=old_value, new_value=new_value, ip_address=ip_address, user_agent=user_agent, metadata=metadata)
    ...
    async def get_entity_history(self, entity_type: str, entity_id: str, limit: int=100) -> list[AuditEntry]:
    ...
    def get_record_count(self) -> int:
        ...
        try:
            ...
            raise RuntimeError('get_record_count() cannot be used inside an active event loop; use await get_record_count_async() instead')

    async def search_incidents(self, query: str, incident_type: str | None=None, status: str | None=None, limit: int=100) -> list[AuditEntry]:
        """
        ...
        return await self._repo.search(query=query, action=action, entity_type=incident_type, limit=limit)

    async def search_actions(self, action: str | None=None, user_id: str | None=None, entity_type: str | None=None, start_date: datetime | None=None, end_date: datetime | None=None, limit: int=100) -> list[AuditEntry]:
