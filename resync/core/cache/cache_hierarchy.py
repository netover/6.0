...
class L1Cache:
    ...
    def __init__(self, max_size: int=1000, num_shards: int=16):
        ...
        self.shards: list[LRUCache] = [LRUCache(maxsize=max_size // num_shards if num_shards > 0 else max_size) for _ in range(num_shards)]
...
class CacheHierarchy:
    ...
    def __init__(self, l1_max_size: int=1000, l2_ttl_seconds: int=300, l2_cleanup_interval: int=30, enable_encryption: bool=False, key_prefix: str='cache:'):
        """
        ...
        self.l2_cache = AsyncTTLCache(ttl_seconds=l2_ttl_seconds, cleanup_interval=l2_cleanup_interval)
        self.metrics = CacheMetrics()
        self.is_running = False
        logger.info(f'CacheHierarchy initialized: L1_max_size={l1_max_size}, L2_ttl={l2_ttl_seconds}s, encryption={enable_encryption}, key_prefix={key_prefix}')
    ...
    def _encrypt_value(self, value: Any) -> Any:
        ...
        if self.enable_encryption:
            try:
                ...
                logger.warning('encryption_failed', extra={'error': str(e), 'value_type': type(value).__name__})
        return value
    ...
    def _decrypt_value(self, value: Any) -> Any:
        ...
        if self.enable_encryption and isinstance(value, dict) and value.get('__encrypted__'):
            try:
                ...
                logger.warning('decryption_failed', extra={'error': str(e)})
        return value
    ...
    async def set(self, key: str, value: Any, ttl_seconds: int | None=None) -> None:
        ...
        logger.debug('cache_hierarchy_set', extra={'key': prefixed_key})
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        return {'l1_size': l1_size, 'l2_size': l2_size, 'l1_hit_ratio': self.metrics.l1_hit_ratio, 'l2_hit_ratio': self.metrics.l2_hit_ratio, 'overall_hit_ratio': self.metrics.overall_hit_ratio, 'total_gets': self.metrics.total_gets, 'total_sets': self.metrics.total_sets, 'l1_evictions': self.metrics.l1_evictions}
...
def get_cache_hierarchy() -> CacheHierarchy:
    ...
    if cache_hierarchy is None:
        cache_hierarchy = CacheHierarchy(l1_max_size=settings.CACHE_HIERARCHY.L1_MAX_SIZE, l2_ttl_seconds=settings.CACHE_HIERARCHY.L2_TTL_SECONDS, l2_cleanup_interval=settings.CACHE_HIERARCHY.L2_CLEANUP_INTERVAL, enable_encryption=getattr(settings.CACHE_HIERARCHY, 'CACHE_ENCRYPTION_ENABLED', False), key_prefix=getattr(settings.CACHE_HIERARCHY, 'CACHE_KEY_PREFIX', 'cache:'))
