        ...
        async with self.session_factory() as session:
            ...
            try:
                ...
                while True:
                    ...
                    completed_step_ids = {
                        run.step_id for run in step_runs if run.status == "completed"
                    }
                    ...
                    # Plan next steps
                    batches = await strategy.plan_execution(
                        workflow_config, completed_step_ids
                    )
                    ...
                    if not batches:
                        ...
                        if completed_step_ids >= all_config_ids:
                            ...
                            logger.warning(
                                f"Plan returned empty but not all steps done. Completed: {completed_step_ids}, Total: {all_config_ids}"
                            )
                            break
