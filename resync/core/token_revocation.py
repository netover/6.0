...
- If Valkey is available, store revoked JTIs with TTL until token expiry.
- If Valkey is not configured, revocation is disabled (fail-open) and should be
...
async def revoke_jti(jti: str, exp_unix: int | None = None) -> None:
    ...
    try:
        ...
        if ttl:
            ...
            await valkey.set(key, "1", ex=3600)
    except (OSError, ValueError, TypeError, KeyError, AttributeError, RuntimeError, TimeoutError, ConnectionError) as e:
        import sys as _sys
        from resync.core.exception_guard import maybe_reraise_programming_error
        _exc_type, _exc, _tb = _sys.exc_info()
