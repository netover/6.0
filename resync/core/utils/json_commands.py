...
class JSONParseCommand:
    ...
    def _parse_json(self) -> None:
        ...
        try:
            ...
            logger.warning('JSON decode error', extra={'error': str(e), 'json_length': len(self.json_str)})
    ...
    def _validate_keys(self) -> None:
        ...
        if self.strict and self.required_keys:
            ...
            if extra_keys:
                raise ParsingError(f"JSON contains unexpected keys in strict mode: {', '.join(extra_keys)}")
...
class JSONParseCommandFactory:
    ...
    @staticmethod
    def create_command(text: str, required_keys: list[str], max_size: int=MAX_JSON_SIZE, strict: bool=True) -> JSONParseCommand:
...
class JSONParseCommandExecutor:
    ...
    def execute_command(text: str, required_keys: list[str], max_size: int=MAX_JSON_SIZE, strict: bool=True) -> dict[str, Any]:
