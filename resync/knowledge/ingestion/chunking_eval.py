...
class EvalResult:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {'query_id': self.query_id, 'query_text': self.query_text, 'expected_answer': self.expected_answer, 'actual_answer': self.actual_answer, 'retrieved_chunks': [{'chunk_id': c.chunk_id, 'content': c.content[:200] + '...' if len(c.content) > 200 else c.content, 'score': c.score, 'rank': c.rank} for c in self.retrieved_chunks], 'failure_slice': self.failure_slice.value, 'failure_severity': self.failure_severity.value, 'failure_description': self.failure_description, 'relevant_chunk_ids': self.relevant_chunk_ids, 'retrieved_relevant': self.retrieved_relevant, 'recall_at_k': self.recall_at_k, 'mrr': self.mrr, 'evaluated_at': self.evaluated_at}
...
class RuleSuggestion:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {'failure_slice': self.failure_slice.value, 'current_rule': self.current_rule, 'suggested_rule': self.suggested_rule, 'rationale': self.rationale, 'affected_queries': self.affected_queries, 'estimated_impact': self.estimated_impact}
...
class EvalReport:
    ...
    def to_dict(self) -> dict[str, Any]:
        """Convert to dictionary for storage."""
        return {'total_queries': self.total_queries, 'successful_queries': self.successful_queries, 'failed_queries': self.failed_queries, 'failure_slice_counts': self.failure_slice_counts, 'severity_counts': self.severity_counts, 'avg_recall': self.avg_recall, 'avg_mrr': self.avg_mrr, 'rule_suggestions': [s.to_dict() for s in self.rule_suggestions], 'evaluated_at': self.evaluated_at}

def detect_failure_slice(query: str, retrieved_chunks: list[RetrievedChunk], expected_answer: str, relevant_chunk_ids: list[str]) -> tuple[FailureSlice, str]:
    """
...
FAILURE_SLICE_RULES: dict[FailureSlice, dict[str, Any]] = {FailureSlice.MISSING_EXCEPTION: {'current_rule': 'Split at paragraph boundaries', 'suggested_rule': "Keep 'rule + exception' pairs together; increase overlap at 'unless', 'except', 'however'", 'impact': 'high'}, FailureSlice.WRONG_SCOPE_VERSION: {'current_rule': 'Index all content equally', 'suggested_rule': 'Add metadata filtering for environment/version; boost non-deprecated content', 'impact': 'high'}, FailureSlice.LOST_TABLE_HEADER: {'current_rule': 'Split tables by rows', 'suggested_rule': 'Keep header + row groups together; add header context to each row chunk', 'impact': 'high'}, FailureSlice.REDUNDANT_OVERLAPS: {'current_rule': 'Constant token overlap', 'suggested_rule': 'Use structure-aware overlap at paragraph/heading boundaries only', 'impact': 'medium'}, FailureSlice.NEEDS_CROSS_SECTION: {'current_rule': 'Single chunk retrieval', 'suggested_rule': 'Enable hierarchical chunking; retrieve parent context for cross-section queries', 'impact': 'high'}, FailureSlice.MISSING_PROCEDURE_STEP: {'current_rule': 'Split procedures by token count', 'suggested_rule': 'Keep complete procedures together; split only at step boundaries', 'impact': 'high'}, FailureSlice.CODE_WITHOUT_CONTEXT: {'current_rule': 'Split code blocks independently', 'suggested_rule': 'Keep code + preceding explanation together; add code summary to metadata', 'impact': 'medium'}, FailureSlice.ERROR_CODE_INCOMPLETE: {'current_rule': 'Split error docs by token count', 'suggested_rule': 'Keep error code + cause + solution as atomic unit', 'impact': 'high'}, FailureSlice.DEFINITION_TRUNCATED: {'current_rule': 'Split at sentence boundaries', 'suggested_rule': 'Detect definition patterns and keep complete; include examples', 'impact': 'medium'}, FailureSlice.LIST_ITEM_ORPHANED: {'current_rule': 'Split lists by items', 'suggested_rule': 'Keep list intro + items together; add intro context to each item', 'impact': 'medium'}}
...
def generate_rule_suggestions(results: list[EvalResult]) -> list[RuleSuggestion]:
    ...
    for result in results:
        ...
        if slice_type not in suggestions:
            ...
            suggestions[slice_type] = RuleSuggestion(failure_slice=slice_type, current_rule=rule_info.get('current_rule', 'Unknown'), suggested_rule=rule_info.get('suggested_rule', 'Review and adjust'), rationale=f'Detected in {result.query_id}: {result.failure_description}', affected_queries=1, estimated_impact=rule_info.get('impact', 'medium'))
        else:
...
class ChunkingEvalPipeline:
    ...
    async def evaluate_query(self, query_id: str, query_text: str, expected_answer: str, relevant_chunk_ids: list[str], retriever: Any | None=None, retrieved_chunks: list[RetrievedChunk] | None=None) -> EvalResult:
        """
        ...
        if retrieved_chunks is None and retriever is not None:
            try:
                ...
                retrieved_chunks = [RetrievedChunk(chunk_id=r.get('id', r.get('chunk_id', '')), content=r.get('content', r.get('text', '')), score=r.get('score', 0.0), rank=i + 1, metadata=r.get('metadata', {})) for i, r in enumerate(raw_results)]
            except Exception as e:
                logger.error('eval_retrieve_failed', extra={'query_id': query_id, 'error': str(e)})
                retrieved_chunks = []
        ...
        for chunk in retrieved_chunks:
            if chunk.chunk_id in relevant_set:
                mrr = 1.0 / chunk.rank
                break
        failure_slice, failure_desc = detect_failure_slice(query_text, retrieved_chunks, expected_answer, relevant_chunk_ids)
        ...
        if recall == 0:
            ...
            severity = FailureSeverity.MEDIUM
        return EvalResult(query_id=query_id, query_text=query_text, expected_answer=expected_answer, retrieved_chunks=retrieved_chunks, top_k_used=self.top_k, failure_slice=failure_slice, failure_severity=severity, failure_description=failure_desc, relevant_chunk_ids=relevant_chunk_ids, retrieved_relevant=len(relevant_retrieved) > 0, recall_at_k=recall, mrr=mrr)

    async def evaluate_queries(self, queries: list[dict[str, Any]], retriever: Any | None=None) -> EvalReport:
        """
        ...
        for query in queries:
            result = await self.evaluate_query(query_id=query.get('id', ''), query_text=query.get('text', ''), expected_answer=query.get('expected', ''), relevant_chunk_ids=query.get('relevant_ids', []), retriever=retriever)
    ...
    def _generate_report(self, results: list[EvalResult]) -> EvalReport:
        ...
        return EvalReport(total_queries=total, successful_queries=successful, failed_queries=failed, failure_slice_counts=slice_counts, severity_counts=severity_counts, avg_recall=avg_recall, avg_mrr=avg_mrr, rule_suggestions=suggestions, results=results)
    ...
    def load_results(self, path: str | Path) -> list[EvalResult]:
        ...
        for item in data.get('results', []):
            result = EvalResult(query_id=item['query_id'], query_text=item['query_text'], expected_answer=item['expected_answer'], actual_answer=item.get('actual_answer'), failure_slice=FailureSlice(item.get('failure_slice', 'unknown')), failure_severity=FailureSeverity(item.get('failure_severity', 'medium')), failure_description=item.get('failure_description', ''), relevant_chunk_ids=item.get('relevant_chunk_ids', []), retrieved_relevant=item.get('retrieved_relevant', False), recall_at_k=item.get('recall_at_k', 0.0), mrr=item.get('mrr', 0.0), evaluated_at=item.get('evaluated_at', ''))
            results.append(result)
        return results

def create_eval_query(query_id: str, query_text: str, expected_answer: str, relevant_chunk_ids: list[str]) -> dict[str, Any]:
    """Create a query dict for evaluation."""
    return {'id': query_id, 'text': query_text, 'expected': expected_answer, 'relevant_ids': relevant_chunk_ids}
__all__ = ['FailureSlice', 'FailureSeverity', 'RetrievedChunk', 'EvalResult', 'RuleSuggestion', 'EvalReport', 'ChunkingEvalPipeline', 'detect_failure_slice', 'generate_rule_suggestions', 'create_eval_query', 'FAILURE_SLICE_RULES']