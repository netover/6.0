...
class SupportedFormat(str, Enum):
    ...
    XLSX = 'xlsx'
    TXT = 'txt'
FORMAT_EXTENSIONS: dict[str, SupportedFormat] = {'.pdf': SupportedFormat.PDF, '.docx': SupportedFormat.DOCX, '.doc': SupportedFormat.DOCX, '.html': SupportedFormat.HTML, '.htm': SupportedFormat.HTML, '.md': SupportedFormat.MARKDOWN, '.markdown': SupportedFormat.MARKDOWN, '.xlsx': SupportedFormat.XLSX, '.txt': SupportedFormat.TXT, '.text': SupportedFormat.TXT}
...
def _docling_convert_worker(file_path: str, do_table_structure: bool, do_ocr: bool, doc_timeout: int, result_path: str) -> None:
    """
    ...
    try:
        ...
        converter = DocumentConverter(allowed_formats=[InputFormat.PDF, InputFormat.DOCX, InputFormat.HTML, InputFormat.MD, InputFormat.XLSX], format_options={InputFormat.PDF: PdfFormatOption(pipeline_options=pdf_opts)})
        ...
        for table_item in doc.tables:
            try:
                ...
                rows, cols = (0, 0)
            tables.append({'content_markdown': md_table, 'page': getattr(table_item.prov[0], 'page_no', None) if table_item.prov else None, 'rows': rows, 'cols': cols})
        metadata = {'title': getattr(doc, 'name', '') or Path(file_path).stem, 'num_pages': getattr(result.document, 'num_pages', 0) if hasattr(result.document, 'num_pages') else 0, 'num_tables': len(tables)}
        output = {'status': 'success', 'markdown': markdown, 'tables': tables, 'metadata': metadata}
    except Exception as e:
        output = {'status': 'error', 'error': f'{type(e).__name__}: {str(e)}', 'markdown': '', 'tables': [], 'metadata': {}}
...
def _plaintext_fallback(file_path: str) -> dict:
    ...
    try:
        ...
        return {'status': 'success', 'markdown': text, 'tables': [], 'metadata': {'title': Path(file_path).stem, 'fallback': True}}
    except Exception as e:
        return {'status': 'error', 'error': str(e), 'markdown': '', 'tables': [], 'metadata': {}}
...
class DoclingConverter:
    ...
    def __init__(self, *, do_table_structure: bool=True, do_ocr: bool=False, doc_timeout: int=120, process_timeout: int=300):
        """
        Args:
    ...
    async def convert(self, file_path: str | Path) -> ConvertedDocument:
        ...
        if not file_path.exists():
            return ConvertedDocument(markdown='', source_path=str(file_path), status='error', error=f'File not found: {file_path}')
        if fmt is None:
            return ConvertedDocument(markdown='', source_path=str(file_path), status='error', error=f'Unsupported format: {file_path.suffix}')
        ...
        logger.info('docling_convert_start', extra={'file': str(file_path), 'format': fmt.value})
        ...
        if result.status == 'success':
            logger.info('docling_convert_done', extra={'file': file_path.name, 'pages': result.pages, 'tables': len(result.tables), 'time_s': f'{elapsed:.1f}'})
        else:
            logger.warning('docling_convert_failed', extra={'file': file_path.name, 'error': result.error, 'time_s': f'{elapsed:.1f}'})
        return result

    async def convert_batch(self, file_paths: list[str | Path], max_concurrent: int=2) -> list[ConvertedDocument]:
        """
    ...
    async def _run_in_subprocess(self, file_path: str) -> dict:
        ...
        try:
            ...
            result_data = await loop.run_in_executor(None, self._sync_subprocess, file_path, result_path)
            return result_data
        finally:
            try:
    ...
    def _sync_subprocess(self, file_path: str, result_path: str) -> dict:
        ...
        proc = ctx.Process(target=_docling_convert_worker, args=(file_path, self.do_table_structure, self.do_ocr, self.doc_timeout, result_path), daemon=True)
        proc.start()
        proc.join(timeout=self.process_timeout)
        if proc.is_alive():
            proc.kill()
            proc.join(timeout=5)
            return {'status': 'error', 'error': f'Conversion timed out after {self.process_timeout}s', 'markdown': '', 'tables': [], 'metadata': {}}
        if proc.exitcode != 0:
            logger.warning('docling_subprocess_crashed', extra={'exitcode': proc.exitcode, 'file': file_path})
            return _plaintext_fallback(file_path)
        try:
            with open(result_path, encoding='utf-8') as f:
                return json.load(f)
        except (json.JSONDecodeError, FileNotFoundError) as e:
            return {'status': 'error', 'error': f'Failed to read conversion results: {e}', 'markdown': '', 'tables': [], 'metadata': {}}

    @staticmethod
    def _build_result(data: dict, source_path: str, fmt: str) -> ConvertedDocument:
        """Build ConvertedDocument from subprocess output."""
        tables = [ExtractedTable(content_markdown=t.get('content_markdown', ''), page=t.get('page'), section=t.get('section'), rows=t.get('rows', 0), cols=t.get('cols', 0)) for t in data.get('tables', [])]
        metadata = data.get('metadata', {})
        return ConvertedDocument(markdown=data.get('markdown', ''), tables=tables, metadata=metadata, source_path=source_path, format=fmt, pages=metadata.get('num_pages', 0), status=data.get('status', 'error'), error=data.get('error'))

async def convert_document(file_path: str | Path, *, do_table_structure: bool=True, do_ocr: bool=False) -> ConvertedDocument:
    """
