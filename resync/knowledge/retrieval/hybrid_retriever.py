...
from .reranker_interface import IReranker, RerankGatingConfig, RerankGatingPolicy, create_reranker
...
class BM25Index:
    ...
    k1: float = 1.5
    b: float = 0.75
    field_boosts: dict[str, float] = field(default_factory=lambda: {'job_name': 4.0, 'error_code': 3.5, 'workstation': 3.0, 'job_stream': 2.5, 'message_id': 2.5, 'resource': 2.0, 'title': 1.5, 'content': 1.0})
    ...
    ERROR_CODE_PATTERNS = [re.compile('RC[=:]\\s*(\\d+)', re.IGNORECASE), re.compile('ABEND\\s+([A-Z0-9]+)', re.IGNORECASE), re.compile('EQQQ(\\w+)', re.IGNORECASE), re.compile('AWSB(\\w+)', re.IGNORECASE), re.compile('(?:error|erro)[:\\s]+(\\w+)', re.IGNORECASE)]
    ...
    def build_index(self, documents: list[dict[str, Any]], text_field: str='content') -> None:
        ...
        logger.info(f'BM25 index built with field boosting: {len(documents)} docs, {len(self.inverted_index)} unique terms, avg_length={{self.avg_doc_length:.1f}}')
    ...
    def save(self, path: str) -> bool:
        ...
        try:
            ...
            logger.error('bm25_index_save_failed', extra={'error': str(e), 'path': path})
            return False
    ...
    def load(cls, path: str) -> 'BM25Index':
        ...
        try:
            ...
            logger.warning('bm25_index_corrupted_rebuilding', extra={'error': str(e), 'path': path})
            try:
                ...
                logger.error('bm25_index_cleanup_failed', extra={'error': str(cleanup_error)})
            return cls()
        except MemoryError:
            logger.error('bm25_index_oom_using_empty', extra={'path': path})
            return cls()
        except Exception as e:
            logger.warning('bm25_index_load_failed_unknown', extra={'error': str(e), 'error_type': type(e).__name__, 'path': path})
            return cls()
...
class VectorStore(Protocol):
    ...
    def query(self, vector: list[float], top_k: int, collection: str | None=None, filters: dict[str, Any] | None=None, ef_search: int | None=None, with_vectors: bool=False) -> list[dict[str, Any]]:
        ...

    def get_all_documents(self, collection: str | None=None, limit: int=10000) -> list[dict[str, Any]]:
        ...
...
class QueryClassificationCache:
    ...
    def get(self, query: str) -> QueryClassificationResult | None:
        ...
        return QueryClassificationResult(query_type=result.query_type, vector_weight=result.vector_weight, bm25_weight=result.bm25_weight, cached=True, classification_time_ms=0.0)
...
class HybridRetrieverConfig:
    ...
    def from_settings(cls) -> HybridRetrieverConfig:
        ...
        try:
            ...
            field_boosts = {'job_name': getattr(settings, 'hybrid_boost_job_name', 4.0), 'error_code': getattr(settings, 'hybrid_boost_error_code', 3.5), 'workstation': getattr(settings, 'hybrid_boost_workstation', 3.0), 'job_stream': getattr(settings, 'hybrid_boost_job_stream', 2.5), 'message_id': getattr(settings, 'hybrid_boost_message_id', 2.5), 'resource': getattr(settings, 'hybrid_boost_resource', 2.0), 'title': getattr(settings, 'hybrid_boost_title', 1.5), 'content': getattr(settings, 'hybrid_boost_content', 1.0)}
            return cls(vector_weight=getattr(settings, 'hybrid_vector_weight', 0.5), bm25_weight=getattr(settings, 'hybrid_bm25_weight', 0.5), auto_weight=getattr(settings, 'hybrid_auto_weight', True), field_boosts=field_boosts, cache_enabled=getattr(settings, 'hybrid_cache_enabled', True), cache_max_size=getattr(settings, 'hybrid_cache_max_size', 1000), cache_ttl_seconds=getattr(settings, 'hybrid_cache_ttl_seconds', 3600), metrics_enabled=getattr(settings, 'hybrid_metrics_enabled', True), rerank_gating_enabled=getattr(settings, 'rerank_gating_enabled', True), rerank_score_low_threshold=getattr(settings, 'rerank_score_low_threshold', 0.35), rerank_margin_threshold=getattr(settings, 'rerank_margin_threshold', 0.05), rerank_max_candidates=getattr(settings, 'rerank_max_candidates', 10))
        except ImportError:
            return cls()
...
class HybridRetriever:
    ...
    """
    EXACT_MATCH_PATTERNS = [re.compile('\\b[A-Z]{2,}[0-9]{2,}[A-Z0-9_]*\\b'), re.compile('\\bAWSB[A-Z0-9]+\\b', re.IGNORECASE), re.compile('\\b[A-Z]{2,}_[A-Z0-9_]+_\\d+\\b'), re.compile('\\bJOB[=:]\\s*[A-Z0-9_]+', re.IGNORECASE), re.compile('\\bRC[=:]\\s*\\d+\\b', re.IGNORECASE), re.compile('\\bABEND\\s+[A-Z0-9]+\\b', re.IGNORECASE), re.compile('\\bS[0-9A-F]{3}\\b'), re.compile('\\bU\\d{4}\\b'), re.compile('\\bCC[=:]\\s*\\d+\\b', re.IGNORECASE), re.compile('\\b[A-Z]{2}\\d{3,}\\b'), re.compile('\\bLPAR\\d+\\b', re.IGNORECASE), re.compile('\\bSYSPLEX\\w+\\b', re.IGNORECASE), re.compile('\\bEQQQ[A-Z0-9]+\\b', re.IGNORECASE), re.compile('\\bIEF\\d{3}[A-Z]\\b', re.IGNORECASE), re.compile('\\bIKJ\\d{5}[A-Z]\\b', re.IGNORECASE), re.compile('\\bCSV\\d{3}[A-Z]\\b', re.IGNORECASE), re.compile('\\b[A-Z]+\\.[A-Z0-9]+\\.[A-Z0-9]+\\b'), re.compile('\\bDSN[=:]\\s*\\S+', re.IGNORECASE), re.compile('\\b\\d{1,2}:\\d{2}(?::\\d{2})?\\b'), re.compile('\\b(?:ODATE|SCHED_TIME)[=:]\\s*\\S+', re.IGNORECASE)]
    SEMANTIC_PATTERNS = [re.compile('\\b(como|how|why|por\\s*que|quando|when|onde|where)\\b', re.IGNORECASE), re.compile('\\b(resolver|fix|solve|solucionar|corrigir)\\b', re.IGNORECASE), re.compile('\\b(configurar|configure|setup|instalar|install)\\b', re.IGNORECASE), re.compile('\\b(ajuda|help|suporte|support)\\b', re.IGNORECASE), re.compile('\\b(explicar?|explain|what\\s+is|o\\s+que\\s+[eé]|significa)\\b', re.IGNORECASE), re.compile('\\b(definição|definition|conceito|concept)\\b', re.IGNORECASE), re.compile('\\b(melhores?\\s+práticas?|best\\s+practices?)\\b', re.IGNORECASE), re.compile('\\b(recomenda[çc][ãa]o|recommendation|dica|tip)\\b', re.IGNORECASE), re.compile('\\b(documentação|documentation|manual|guia|guide|tutorial)\\b', re.IGNORECASE), re.compile('\\b(exemplo|example|demonstra[çc][ãa]o|demo)\\b', re.IGNORECASE), re.compile('\\b(analis[ea]r?|analyze|investigar?|investigate)\\b', re.IGNORECASE), re.compile('\\b(comparar?|compare|diferen[çc]a|difference)\\b', re.IGNORECASE)]

    def __init__(self, embedder: Embedder, store: VectorStore, config: HybridRetrieverConfig | None=None):
        ...
        gating_config = RerankGatingConfig(enabled=self.config.rerank_gating_enabled, score_low_threshold=self.config.rerank_score_low_threshold, margin_threshold=self.config.rerank_margin_threshold, max_candidates=self.config.rerank_max_candidates)
        ...
        if self.config.cache_enabled:
            self._classification_cache = QueryClassificationCache(max_size=self.config.cache_max_size, ttl_seconds=self.config.cache_ttl_seconds)
    ...
    def _classify_query(self, query: str) -> QueryClassificationResult:
        ...
        result = QueryClassificationResult(query_type=query_type, vector_weight=weights[0], bm25_weight=weights[1], cached=False, classification_time_ms=classification_time)
        if self._classification_cache:
            self._classification_cache.put(query, result)
        logger.debug(f'Query classified as {query_type.value}: {query[:50]}... (weights: v={weights[0]:.1f}, b={weights[1]:.1f})')
        return result
    ...
    async def _ensure_bm25_index(self, collection: str | None=None) -> None:
        ...
        try:
            import asyncio
            logger.info('Attempting to load persisted BM25 index', extra={'path': index_path})
            self.bm25_index = await asyncio.to_thread(BM25Index.load, index_path)
            if self.bm25_index and self.bm25_index.documents:
                self._index_built = True
                logger.info('BM25 index loaded from disk', extra={'num_docs': len(self.bm25_index.documents)})
                return
        except Exception as e:
            logger.warning('BM25 index load failed, will rebuild', extra={'error': str(e)})
        ...
        try:
            ...
            if documents:
                self.bm25_index = BM25Index(k1=self.config.bm25_k1, b=self.config.bm25_b, field_boosts=self.config.field_boosts)
    ...
    def _reciprocal_rank_fusion(self, results_list: list[list[dict[str, Any]]], weights: list[float], k: int=60) -> list[dict[str, Any]]:
        """
    ...
    async def _vector_search(self, query: str, top_k: int, collection: str | None=None, filters: dict[str, Any] | None=None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            return self.store.query(vector=vec, top_k=top_k, collection=collection, filters=filters, ef_search=ef, with_vectors=False)
        except Exception as e:
    ...
    async def _bm25_search(self, query: str, top_k: int, collection: str | None=None) -> list[dict[str, Any]]:
    ...
    async def retrieve(self, query: str, top_k: int=10, filters: dict[str, Any] | None=None, collection: str | None=None, enable_reranking: bool | None=None) -> list[dict[str, Any]]:
        """
        ...
        logger.debug(f'Hybrid search: vector={len(vector_results)}, bm25={len(bm25_results)}, weights=(v:{{vector_weight:.1f}}, b:{{bm25_weight:.1f}}), type={classification.query_type.value}')
        ...
        if not vector_results:
            ...
            results = self._reciprocal_rank_fusion([vector_results, bm25_results], [vector_weight, bm25_weight])
        ...
        if do_rerank and len(results) > 1:
            scores = [doc.get('rrf_score', 0) or doc.get('score', 0) or doc.get('vector_score', 0) for doc in results]
            ...
            if should_rerank:
                ...
                if len(results) < top_k:
                    remaining = [doc for doc in results[self._gating_policy.config.max_candidates:] if doc not in results]
