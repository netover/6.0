...
def _get_knowledge_graph_or_stub() -> Any:
    ...
    try:
        ...
        class _KGStub:
            ...
            def get_relevant_context(self, query: str) -> str | None:
...
async def router_node(state: AgentState) -> AgentState:
    ...
    if pre_defined_intent and pre_defined_intent != Intent.UNKNOWN and state.get("metadata", {}).get(
        "intent_predefined"
    ):
        ...
        logger.info(
            ...
            intent=pre_defined_intent.value if isinstance(pre_defined_intent, Intent) else pre_defined_intent,
...
async def plan_executor_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        if action == "verify_action" and (state.get("raw_data", {}) or {}).get("_verification_retry"):
            step["completed"] = False
            step["error"] = None
...
async def _execute_orchestrator_collect(state: AgentState, include_logs: bool) -> AgentState:
    ...
    approval = interrupt(
        {
            ...
            "pre_action_status": (state.get("raw_data", {}) or {}).get("pre_action_status"),
        }
    )
...
async def _execute_tws_action(state: AgentState) -> AgentState:
    ...
    state.setdefault("raw_data", {}).update({"action": action_type, "action_result": result})
...
async def output_critique_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        critique = json.loads(m.group(0)) if m else {"satisfactory": True, "issues": [], "missing": []}

        if not critique.get("satisfactory", True):
            state["critique_retries"] = retries + 1
            state["critique_feedback"] = (critique.get("issues") or []) + (critique.get("missing") or [])
...
async def troubleshoot_handler_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        from resync.core.langgraph.incident_response import handle_incident, OutputChannel
...
async def _fallback_troubleshoot(state: AgentState, job_name: str | None, message: str) -> AgentState:
...
async def general_handler_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        if agent_prompt:
            ...
            full_prompt = f"Você é um assistente especialista em TWS/HWA.\n\nUsuário: {message}"
...
def synthesizer_node(state: AgentState) -> AgentState:
    ...
    if intent == Intent.STATUS:
        ...
        state["response"] = render_template(
            ...
            recommendations="\n".join(f"- {r}" for r in recs) if recs else "- Verificar logs",
        )
        ...
        # Generic fallback
        state["response"] = render_template("generic_response", content=json.dumps(raw_data, indent=2))
...
def _should_retry(state: AgentState) -> str:
    ...
    if state.get("tool_error") and state.get("retry_count", 0) < state.get("max_retries", 3):
        return "retry"
    return "synthesizer"
...
def create_tws_agent_graph(
    ...
    for handler in ["query_handler", "general_handler", "status_handler", "troubleshoot_handler", "action_handler"]:
    ...
    if config.enable_output_critique and config.enable_hallucination_check:
        def _after_synthesizer(state: AgentState) -> str:
            return "output_critique" if state.get("intent") == Intent.TROUBLESHOOT else "hallucination_check"

        graph.add_conditional_edges(
            "synthesizer",
            _after_synthesizer,
            {"output_critique": "output_critique", "hallucination_check": "hallucination_check"},
        )

        graph.add_conditional_edges(
            "output_critique",
            _after_critique,
            {"synthesizer": "synthesizer", "hallucination_check": "hallucination_check"},
        )
