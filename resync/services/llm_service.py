...
from resync.core.resilience import CircuitBreaker, CircuitBreakerConfig, RetryConfig, RetryWithBackoff, TimeoutManager
...
try:
    from openai import APIConnectionError, APIError, APIStatusError, APITimeoutError, AsyncOpenAI, AuthenticationError, BadRequestError, RateLimitError
...
class LLMService:
    ...
    def __init__(self) -> None:
        ...
        if not OPENAI_AVAILABLE:
            raise IntegrationError(message='openai package is required but not installed', details={'install_command': 'pip install openai'})
        model = getattr(settings, 'llm_model', None)
        if model is None:
            model = getattr(settings, 'agent_model_name', None)
        if not model:
            raise IntegrationError(message='No LLM model configured', details={'hint': 'Define settings.llm_model or settings.agent_model_name'})
        ...
        if not base_url:
            raise IntegrationError(message='Missing LLM base_url', details={'hint': 'Configure settings.llm_endpoint (NVIDIA OpenAI-compatible)'})
        if api_key:
        ...
        try:
            self.client = AsyncOpenAI(api_key=api_key, base_url=base_url, timeout=float(getattr(settings, 'llm_timeout', 20.0) or 20.0), max_retries=0)
            ...
            self._retry = RetryWithBackoff(RetryConfig(max_retries=int(os.getenv('LLM_MAX_RETRIES', '2')), base_delay=float(os.getenv('LLM_RETRY_BASE_DELAY', '0.5')), max_delay=float(os.getenv('LLM_RETRY_MAX_DELAY', '8.0')), jitter=True, expected_exceptions=(ServiceUnavailableError,)))
            self._cb = CircuitBreaker(CircuitBreakerConfig(failure_threshold=int(os.getenv('LLM_CB_FAILURE_THRESHOLD', '5')), recovery_timeout=int(os.getenv('LLM_CB_RECOVERY_TIMEOUT', '60')), expected_exception=ServiceUnavailableError, exclude_exceptions=(ConfigurationError, IntegrationError), name='llm'))
            logger.info('llm_resilience_configured', extra={'timeout_s': self._timeout_s, 'max_concurrency': self._max_concurrency})
        ...
        except (AuthenticationError, RateLimitError, APIConnectionError, BadRequestError, APIError, APITimeoutError, APIStatusError) as exc:
            logger.error('Failed to initialize LLM service (OpenAI error): %s', exc, exc_info=True)
            raise IntegrationError(message='Failed to initialize LLM service', details={'error': str(exc), 'request_id': getattr(exc, 'request_id', None)}) from exc
            ...
            raise IntegrationError(message='Failed to initialize LLM service', details={'error': str(exc)}) from exc
    ...
    def _translate_openai_error(self, exc: Exception, *, operation: str) -> BaseAppException:
        ...
        if isinstance(exc, AuthenticationError):
            return ConfigurationError(message=f'LLM authentication failed during {operation}', details={'operation': operation, 'request_id': request_id, 'status_code': status_code}, original_exception=exc)
        if isinstance(exc, BadRequestError):
            return IntegrationError(message=f'LLM request rejected during {operation}', details={'operation': operation, 'request_id': request_id, 'status_code': status_code, 'error': str(exc)}, original_exception=exc)
        if isinstance(exc, RateLimitError):
            return ServiceUnavailableError(message=f'LLM rate limited during {operation}', retry_after=self._extract_retry_after_seconds(exc), details={'operation': operation, 'request_id': request_id, 'status_code': status_code}, original_exception=exc)
        if isinstance(exc, (APIConnectionError, APITimeoutError)):
            return ServiceUnavailableError(message=f'LLM network/timeout failure during {operation}', details={'operation': operation, 'request_id': request_id}, original_exception=exc)
        if isinstance(exc, APIStatusError):
            if status_code == 429 or (isinstance(status_code, int) and status_code >= 500):
                return ServiceUnavailableError(message=f'LLM upstream error during {operation}', retry_after=self._extract_retry_after_seconds(exc), details={'operation': operation, 'request_id': request_id, 'status_code': status_code}, original_exception=exc)
            return IntegrationError(message=f'LLM returned non-retriable status during {operation}', details={'operation': operation, 'request_id': request_id, 'status_code': status_code, 'error': str(exc)}, original_exception=exc)
        if isinstance(exc, APIError):
            return ServiceUnavailableError(message=f'LLM API error during {operation}', details={'operation': operation, 'request_id': request_id, 'status_code': status_code}, original_exception=exc)
        return IntegrationError(message=f'Unexpected LLM error during {operation}', details={'operation': operation, 'request_id': request_id, 'status_code': status_code, 'error': str(exc)}, original_exception=exc)
    ...
    async def _call_openai(self, operation: str, coro_factory: Any, *, retry: bool=True) -> Any:
        ...
        async def _protected() -> Any:
            async with self._sem:
                try:
                    return await TimeoutManager.with_timeout(coro_factory(), timeout_seconds=self._timeout_s, timeout_exception=ServiceUnavailableError(message=f'LLM operation timed out during {operation}', details={'operation': operation, 'timeout_s': self._timeout_s}))
                except (AuthenticationError, RateLimitError, APIConnectionError, BadRequestError, APIError, APITimeoutError, APIStatusError) as exc:
    ...
    async def _chat_completion(self, *, operation: str, messages: list[dict[str, str]] | list[dict[str, Any]], stream: bool=False, **kwargs: Any) -> Any:
        """Helper around ``chat.completions.create`` with resilience."""
        return await self._call_openai(operation, lambda: self.client.chat.completions.create(model=self.model, messages=messages, stream=stream, **kwargs), retry=not stream)

    async def generate_response_with_tools(self, messages: list[dict[str, str]], user_role: str='operator', user_id: str | None=None, session_id: str | None=None, max_tool_iterations: int=5, temperature: float | None=None, max_tokens: int | None=None) -> str:
        """
        ...
        try:
            ...
            role_map = {'viewer': UserRole.VIEWER, 'operator': UserRole.OPERATOR, 'admin': UserRole.ADMIN, 'system': UserRole.SYSTEM}
            user_role_enum = role_map.get(user_role.lower(), UserRole.OPERATOR)
            tools = get_llm_tools(user_role=user_role_enum)
            logger.info(f'Generating response with {len(tools)} tools available for role {user_role}')
            current_messages = messages.copy()
            temp = temperature if temperature is not None else 0.3
            max_tok = max_tokens if max_tokens is not None else self.default_max_tokens
            for iteration in range(max_tool_iterations):
                response = await self._chat_completion(operation='tool_completion', messages=current_messages, tools=tools if tools else None, temperature=temp, max_tokens=max_tok)
                message = response.choices[0].message
                if not message.tool_calls:
                    return message.content or ''
                logger.debug(f'LLM requested {len(message.tool_calls)} tool calls at iteration {iteration}')
                current_messages.append({'role': 'assistant', 'content': message.content, 'tool_calls': [{'id': tc.id, 'type': 'function', 'function': {'name': tc.function.name, 'arguments': tc.function.arguments}} for tc in message.tool_calls]})
                for tool_call in message.tool_calls:
                    result = await execute_tool_call(tool_call, user_id=user_id, user_role=user_role_enum, session_id=session_id)
                    current_messages.append({'role': 'tool', 'tool_call_id': tool_call.id, 'name': tool_call.function.name, 'content': json.dumps(result, ensure_ascii=False)})
    ...
    async def generate_response(self, messages: list[dict[str, str]], temperature: float | None=None, top_p: float | None=None, max_tokens: int | None=None, frequency_penalty: float | None=None, presence_penalty: float | None=None, stream: bool=False) -> str:
        """
        ...
        presence_penalty = self.default_presence_penalty if presence_penalty is None else presence_penalty
        logger.info('Generating LLM response with model: %s', self.model)
        try:
            if stream:
                chunks: list[str] = []
                async for piece in self._generate_response_streaming(messages, temperature, top_p, max_tokens, frequency_penalty, presence_penalty):
                    chunks.append(piece)
                content = ''.join(chunks)
            else:
                response = await self._chat_completion(operation='chat_completion', messages=messages, temperature=temperature, top_p=top_p, max_tokens=max_tokens, frequency_penalty=frequency_penalty, presence_penalty=presence_penalty, stream=False)
            ...
            raise IntegrationError(message='Failed to generate LLM response', details={'error': str(exc), 'request_id': getattr(exc, 'request_id', None), 'model': self.model}) from exc
        except Exception as exc:
            ...
            raise IntegrationError(message='Failed to generate LLM response', details={'error': str(exc), 'model': self.model}) from exc

    async def _generate_response_streaming(self, messages: list[dict[str, str]], temperature: float, top_p: float, max_tokens: int, frequency_penalty: float, presence_penalty: float) -> AsyncGenerator[str, None]:
        """
        ...
        try:
            response = await self._chat_completion(operation='chat_completion_stream', messages=messages, temperature=temperature, top_p=top_p, max_tokens=max_tokens, frequency_penalty=frequency_penalty, presence_penalty=presence_penalty, stream=True)
            ...
            raise IntegrationError(message='Failed to generate streaming LLM response', details={'error': str(exc), 'request_id': getattr(exc, 'request_id', None), 'model': self.model}) from exc
        except Exception as exc:
            if isinstance(exc, (BaseAppException, SystemExit, KeyboardInterrupt, asyncio.CancelledError, TypeError, AttributeError)):
                raise
            logger.error('Unexpected error generating streaming LLM response: %s', exc, exc_info=True)
            raise IntegrationError(message='Failed to generate streaming LLM response', details={'error': str(exc), 'model': self.model}) from exc

    async def generate_agent_response(self, agent_id: str, user_message: str, conversation_history: list[dict[str, str]] | None=None, agent_config: dict[str, Any] | None=None) -> str:
        """
        ...
        if LANGFUSE_INTEGRATION:
            try:
                ...
                if prompt:
                    ...
                    logger.debug('prompt_loaded_from_manager', extra={'prompt_id': prompt.id, 'agent_id': agent_id})
            except Exception as e:
                if isinstance(e, (SystemExit, KeyboardInterrupt, asyncio.CancelledError)):
                    raise
                logger.warning('prompt_manager_fallback', extra={'error': str(e)})
        ...
        if LANGFUSE_INTEGRATION:
            ...
            async with tracer.trace('generate_agent_response', model=self.model) as trace:
                ...
                trace.input_tokens = sum((_count_tokens(m.get('content', '')) for m in messages))
                trace.output_tokens = _count_tokens(response)
                return response
        return await self.generate_response(messages, max_tokens=800)

    async def generate_rag_response(self, query: str, context: str, conversation_history: list[dict[str, str]] | None=None, source_name: str='the documentation', use_opinion_based: bool=True) -> str:
        """
        ...
        if use_opinion_based:
            formatted = self._prompt_formatter.format_rag_prompt(query=query, context=context, source_name=source_name, strict_mode=True, language='pt')
            messages: list[dict[str, str]] = [{'role': 'system', 'content': formatted['system']}]
            if conversation_history:
            ...
            if LANGFUSE_INTEGRATION:
                try:
                    ...
                    if prompt:
                        ...
                        logger.debug('rag_prompt_loaded_from_manager', extra={'prompt_id': prompt.id})
                except Exception as e:
                    if isinstance(e, (SystemExit, KeyboardInterrupt, asyncio.CancelledError)):
                        raise
                    logger.warning('rag_prompt_manager_fallback', extra={'error': str(e)})
            if not system_message:
        ...
        if random.random() < self_rag_sample_rate:
            is_grounded, reflection = await self._check_hallucination(query=query, context=context, response=response)
        if not is_grounded:
            logger.warning('self_rag_hallucination_detected', extra={'query': query[:50], 'reflection': reflection})
            try:
        ...
        if LANGFUSE_INTEGRATION:
            tracer = get_tracer()
            async with tracer.trace('generate_rag_response', model=self.model, prompt_id='rag', metadata={'opinion_based': use_opinion_based, 'self_rag_grounded': is_grounded, 'self_rag_reflection': reflection}) as trace:
                trace.output = response
                trace.input_tokens = sum((_count_tokens(m.get('content', '')) for m in messages))
                trace.output_tokens = _count_tokens(response)
                return response
    ...
    async def _check_hallucination(self, query: str, context: str, response: str) -> tuple[bool, str]:
        """
        ...
        try:
            ...
            reflection = await self.generate_response(messages=[{'role': 'user', 'content': check_prompt}], max_tokens=100, temperature=0.1)
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            try:
                ...
                return {'status': 'healthy', 'model': self.model, 'endpoint': getattr(settings, 'llm_endpoint', None), 'available_models': len(models_resp.data)}
            except Exception:
                await self.generate_response(messages=[{'role': 'user', 'content': 'hi'}], max_tokens=1)
                return {'status': 'healthy', 'model': self.model, 'endpoint': getattr(settings, 'llm_endpoint', None)}
        except (AuthenticationError, RateLimitError, APIConnectionError, BadRequestError, APIError, APITimeoutError, APIStatusError) as exc:
            return {'status': 'unhealthy', 'model': self.model, 'endpoint': getattr(settings, 'llm_endpoint', None), 'error': str(exc), 'request_id': getattr(exc, 'request_id', None)}
        except Exception as exc:
            ...
            return {'status': 'unhealthy', 'model': self.model, 'endpoint': getattr(settings, 'llm_endpoint', None), 'error': str(exc)}
