...
class AdminTuning {
    ...
    async loadAuditTab() {
        this.setActiveTab('tab-overview');
        const container = document.getElementById('tuning-content');
        container.innerHTML = '<div class="stat-label">Loading audit log...</div>';

        try {
            const res = await this.api.get(`${this.basePath}/audit?limit=50`);
            const items = res.audit_log || [];

            if (items.length === 0) {
                container.innerHTML = '<div class="stat-label">No audit entries found.</div>';
                return;
            }
                ...
                <table class="data-table" style="width: 100%; border-collapse: collapse;">
                    <thead>
                        <tr style="background: rgba(0,0,0,0.02); border-bottom: 1px solid var(--border);">
                            ...
                            <th style="padding: 1rem; text-align: left;">Old Value</th>
                            <th style="padding: 1rem; text-align: left;">New Value</th>
                            <th style="padding: 1rem; text-align: left;">Reason</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${items.map(item => this.renderAuditRow(item)).join('')}
                    </tbody>
                </table>
            `;
    ...
    renderAuditRow(item) {
        ...
        const threshold = escapeHtml(item.threshold_name || '');
        ...
        return `
            <tr>
                <td style="padding: 1rem; border-bottom: 1px solid var(--border);">${timestamp}</td>
                <td style="padding: 1rem; border-bottom: 1px solid var(--border); font-weight: 500;">${threshold}</td>
                <td style="padding: 1rem; border-bottom: 1px solid var(--border);">${oldValue}</td>
                <td style="padding: 1rem; border-bottom: 1px solid var(--border);">${newValue}</td>
                <td style="padding: 1rem; border-bottom: 1px solid var(--border);">${reason}</td>
            </tr>
        `;
    }
