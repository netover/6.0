...
def mock_llm():
    ...
    llm = AsyncMock()
    llm.generate = AsyncMock(return_value="variação 1 da pergunta\nvariação 2 da pergunta\nvariação 3 da pergunta")
    return llm
...
class TestFusionRAGRouting:
    ...
    async def test_general_intent_uses_fusion(self, hybrid_rag, mock_rag):
        ...
        with patch.object(hybrid_rag._classifier, 'classify') as mock_classify:
            ...
            from resync.knowledge.retrieval.hybrid import QueryClassification, QueryIntent
            ...
            result = await hybrid_rag.query("pergunta genérica", generate_response=False)
            
            # Should have fusion stats (indicating Fusion RAG was used)
            if result.get("rag_results"):
                assert "fusion_stats" in result["rag_results"] or "type" in result["rag_results"]
    ...
    async def test_documentation_intent_uses_standard_rag(self, hybrid_rag, mock_rag):
        ...
        with patch.object(hybrid_rag._classifier, 'classify') as mock_classify:
            ...
            result = await hybrid_rag.query("como configurar TWS?", generate_response=False)
