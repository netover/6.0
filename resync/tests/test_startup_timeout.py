...
async def test_startup_timeout_triggered():
    ...
    with patch("resync.core.startup.get_settings") as mock_get_settings:
        ...
        with patch(
            "resync.core.startup.run_startup_checks", side_effect=slow_startup_checks
        ):
            with pytest.raises(
                ConfigurationError, match="startup exceeded 0.5s timeout"
            ):
        ...
        with patch(
            ...
            with patch("resync.core.startup.enforce_startup_policy"):
                with patch("resync.core.wiring.init_domain_singletons"):
                    with patch(
                        "resync.core.types.app_state.enterprise_state_from_app"
                    ) as mock_st_getter:
                        mock_st = MagicMock()
                        mock_st_getter.return_value = mock_st

                        with patch(
                            "resync.core.tws_monitor.get_tws_monitor", AsyncMock()
                        ):
                            with patch(
                                "resync.api_gateway.container.setup_dependencies"
                            ):
                                # For internal methods of factory, we use patch.object
                                with patch(
                                    "resync.core.startup._init_proactive_monitoring",
                                    AsyncMock(),
                                ):
                                    with patch(
                                        "resync.core.startup._init_metrics_collector",
                                        AsyncMock(),
                                    ):
                                        with patch(
                                            "resync.core.startup._init_cache_warmup",
                                            AsyncMock(),
                                        ):
                                            with patch(
                                                "resync.core.startup._init_graphrag",
                                                AsyncMock(),
                                            ):
                                                with patch(
                                                    "resync.core.startup._init_config_system",
                                                    AsyncMock(),
                                                ):
                                                    async with app_lifespan(app):
                                                        assert (
                                                            mock_st.startup_complete
                                                            is True
                                                        )
...
async def test_individual_helper_timeout():
    ...
    with patch("resync.core.startup.get_settings") as mock_get_settings:
        ...
        with patch(
            "resync.core.startup.run_startup_checks",
            AsyncMock(return_value={"overall_health": True}),
        ):
            with patch("resync.core.wiring.init_domain_singletons"):
                with patch(
                    "resync.core.types.app_state.enterprise_state_from_app",
                    ...
                    with patch("resync.core.tws_monitor.get_tws_monitor", AsyncMock()):
                        with patch("resync.api_gateway.container.setup_dependencies"):
                            ...
                            with patch(
                                "resync.core.task_tracker.create_tracked_task",
                                side_effect=very_slow_task,
                            ):
