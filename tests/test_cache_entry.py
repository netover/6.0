...
def test_cache_entry_from_dict_defaults():
    ...
    data = {"query": "minimal query", "response": "minimal response"}
...
def test_cache_entry_from_dict_malformed_values_use_fallbacks():
    """Test malformed serialized values gracefully fall back to safe defaults."""
    ...
    assert entry.query == "fallback query"
    assert entry.embedding == []
    assert entry.timestamp == datetime(1970, 1, 1, tzinfo=timezone.utc)
    assert entry.hit_count == 0
    assert entry.metadata == {}
